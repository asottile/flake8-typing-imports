...
def Version_str(self: Version) -> str:
    return f'{self.major}.{self.minor}.{self.patch}'


def Version_new(major: int = 0, minor: int = 0, patch: int = 0) -> Version:
    return Version(major, minor, patch)


def Version_parse(s: str) -> Version:
    return Version_new(*[int(p) for p in s.split('.')])
...
class Plugin:
    ...
    def parse_options(cls, options: Any) -> None:
        ...
        for part in cfg['options']['python_requires'].split(','):
            part = part.strip()
            if part.startswith('>='):
                v = Version_parse(part[2:])

        v = max(v, SYMBOLS[0][0])
        if v not in VERSIONS:
            v_s = Version_str(v)
            raise ValueError(f'min-python-version ({v_s}): unknown version')
    ...
    def run(self) -> Generator[Tuple[int, int, str, Type[Any]], None, None]:
        ...
        for (line, col, k), versions in error_versions.items():
            versions_s = ', '.join(Version_str(v) for v in versions)
